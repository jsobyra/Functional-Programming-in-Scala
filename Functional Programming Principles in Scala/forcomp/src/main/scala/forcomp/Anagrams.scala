...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(x => x.toLower).mapValues(_.size).toList.sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if(occurrences.isEmpty) List(List())
      ...
      val combination = combinations(occurrences.tail)
      (for{
        comb <- combination
        n <- 1 to head._2
      } yield (head._1, n) :: comb) ++ combination
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.map(occurence => (occurence._1, occurence._2 - y.toMap.getOrElse(occurence._1, 0)))
