...
object Huffman {
   ...
   */
    ...
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case tree1 :: tree2 :: rest => (makeCodeTree(tree1, tree2) :: rest).sortWith((t1, t2) => weight(t1) < weight(t2))
      case tree => tree
  }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def encodeLetter(tree: CodeTree)(c: Char): List[Bit] = tree match {
        case Leaf(_, _) => List()
        case Fork(left, _, _, _) if(chars(left).contains(c)) => 0 :: encodeLetter(left)(c)
        case Fork(_, right, _, _) => 1 :: encodeLetter(right)(c)
      }

      text.flatMap(c => encodeLetter(tree)(c))
    }
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
      ...
      a.map(code => prepend(0)(code)) ::: b.map(code => prepend(1)(code))
    }
