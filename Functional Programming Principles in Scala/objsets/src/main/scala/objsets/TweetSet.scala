...
class Empty extends TweetSet {
  ...
  override def mostRetweeted: Tweet = throw new Exception("ERROR")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    ...
    if(right.isEmpty && left.isEmpty) elem
      ...
      val maxPart = max(left.mostRetweeted, right.mostRetweeted)
      max(elem, maxPart)
    }
  }

  override def descendingByRetweet: TweetList = {
    val mostReetwet = mostRetweeted
    new Cons(mostReetwet, remove(mostReetwet).descendingByRetweet)
  }
