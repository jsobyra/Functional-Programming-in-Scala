...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def helper(columnNames: List[String], primary: List[Column], working: List[Column], leisure: List[Column]): (List[Column], List[Column], List[Column]) = {
      columnNames match {
        case Nil => (primary, working, leisure)
        case name :: rest =>
          if(name.startsWith("t01") || name.startsWith("t03") || name.startsWith("t11") || name.startsWith("t1801")
            || name.startsWith("t1803")) helper(rest, col(name) :: primary, working, leisure)
          else if(name.startsWith("t05") || name.startsWith("t1805")) helper(rest, primary, col(name) :: working, leisure)
          else if(name.startsWith("t02") || name.startsWith("t04") || name.startsWith("t06") || name.startsWith("t07")
            ...
            || name.startsWith("t18")) helper(rest, primary, working, col(name) :: leisure)
          else helper(rest, primary, working, leisure)
      }
    }
    ...
    helper(columnNames, primary, working, leisure)
  }
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed
      ...
      .agg(round(avg(summed("primaryNeeds")), 1).as("primaryNeeds"), round(avg(summed("work")), 1).as("work"),
        round(avg(summed("other")), 1).as("other"))
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      ...
      .agg(
        round(avg[TimeUsageRow](_.primaryNeeds), 1).as(Encoders.DOUBLE),
        round(avg[TimeUsageRow](_.work), 1).as(Encoders.DOUBLE),
        round(avg[TimeUsageRow](_.other), 1).as(Encoders.DOUBLE)
      )
